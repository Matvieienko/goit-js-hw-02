...
іншому випадку функція має повертати рядок "You ordered `<quantity>` droids
...
`"You ordered 5 droids worth 15000 credits!"` Виклик
`makeTransaction(3, 1000, 15000)` повертає
`"You ordered 3 droids worth 3000 credits!"` Виклик
`makeTransaction(10, 5000, 8000)` повертає `"Insufficient funds!"` Виклик
`makeTransaction(8, 2000, 10000)` повертає `"Insufficient funds!"` Виклик
`makeTransaction(10, 500, 5000)` повертає
...
рядок до maxLength символів, додає трикрапку **"..."** в кінці та повертає
обрізану версію.
...
- Виклик функції
  `formatMessage("Nunc sed turpis a felis in nunc fringilla", 15)` повертає
  `"Nunc sed turpis..."`
- Виклик функції
  `formatMessage("Nunc sed turpis a felis in nunc fringilla", 41)` повертає
...
true Якщо в рядку відсутні заборонені слова, функція повертає буль false
...
- Виклик функції `checkForSpam("Get rid of sPaM emails. Our book in on sale!")`
  повертає `true`
- Виклик функції `checkForSpam("[SPAM] How to earn fast money?")` повертає
  `true`
...
- Виклик `getShippingCost("Australia")` повертає
  `"Shipping to Australia will cost 170 credits"`
- Виклик `getShippingCost("Germany")` повертає
  `"Sorry, there is no delivery to your country"`
- Виклик `getShippingCost("China")` повертає
  `"Shipping to China will cost 100 credits"`
- Виклик `getShippingCost("Chile")` повертає
  `"Shipping to Chile will cost 250 credits"`
- Виклик `getShippingCost("Jamaica")` повертає
  `"Shipping to Jamaica will cost 120 credits"`
- Виклик `getShippingCost("Sweden")` повертає
  `"Sorry, there is no delivery to your country"`
